...
export abstract class BaseKernel<TExecuteOptions extends KernelExecuteOptions = KernelExecuteOptions> {
	...
	async shutdown(options?: KernelShutdownOptions): Promise<KernelShutdownResult> {
		...
		const exited = await this.#waitForExitWithTimeout(timeoutMs);
		let confirmed = exited !== null;
		if (owned.length > 0) {
		...
		if (!confirmed) {
			// Keep the survivor visible; do not recover ownership from a PID file.
			logger.warn(`${this.#options.languageName} kernel cleanup unconfirmed; no PID/group fallback`, {
				kernelId: this.id,
				pid: proc.pid,
			});
		}
